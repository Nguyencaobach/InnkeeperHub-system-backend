...
export const createProduct = async (req, res) => {
    try {
        ...
        logActivity(req.user, 'CREATE', 'Sản phẩm', result.name);
        await invalidateCache('products:*', 'customer:*products:*');
...
export const updateProduct = async (req, res) => {
    try {
        ...
        logActivity(req.user, 'UPDATE', 'Sản phẩm', result.name);
        await invalidateCache('products:*', 'customer:*products:*');
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        logActivity(req.user, 'DELETE', 'Sản phẩm', result?.name || `ID: ${id}`);
        await invalidateCache('products:*', 'customer:*products:*');
